...
function library:New(name) 
	...
	but.Modal = true 
	but.Text = "" 
	but.BackgroundTransparency = 2 
	but.Parent = Ova 

	local cursor = INST("ImageLabel") 
	cursor.Name = "cursor" 
	cursor.Parent = Ova 
	cursor.BackgroundTransparency = 2 
	...
	Menu.Name = "Menu" 
	Menu.Parent = Ova 
	Menu.BackgroundColor3 = COL3RGB(16, 16, 16) 
	Menu.Position = UDIM2(0.5, -300, 0.5, -300) 
	Menu.Size = UDIM2(0, 600, 0, 610) 
	Menu.Image = "http://www.roblox.com/asset/?id=7284655861" 
	...
	do 
		...
		TextLabel.TextColor3 = COL3RGB(211, 195, 152) 
	...
	function keybindadd(text) 
		if not KeybindList.TextLabel.Frame:FindFirstChild(text) then 
			...
			TextLabel.Font = Enum.Font.SourceSansSemibold 
			TextLabel.Text = text 
			TextLabel.TextColor3 = COL3RGB(10, 10, 10) 
			TextLabel.TextSize = 14.000 
			TextLabel.Name = text 
	...
	function menu:Tab(text) 
		...
		function Tab:MSector(text, side) 
			...
			SectionText.TextColor3 = COL3RGB(10, 10, 10) 
			...
			function MSector:Tab(text) 
				...
				function tab:Element(type, text, data, callback) 
					...
					if type == "Jumbobox" then 
						...
						Button.Text = "" 
						Button.TextColor3 = COL3RGB(10, 10, 10) 
					...
					elseif type == "Slider" then 
						...
						UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, COL3RGB(153, 22, 65)), ColorSequenceKeypoint.new(1, COL3RGB(135, 25, 255))} 
		...
		function Tab:Sector(text, side) 
			...
			function Sector:Element(type, text, data, callback) 
				...
				elseif type == "Slider" then 
					...
					UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, COL3RGB(135, 25, 255)), ColorSequenceKeypoint.new(1, COL3RGB(135, 25, 255))} 
					UIGradient.Rotation = 90 
					UIGradient.Parent = Frame 
...
do 
	...
	Frame.BackgroundColor3 = COL3RGB(167, 29, 196) 
	Frame.BorderSizePixel = 0 
	Frame.Position = UDIM2(0, 0, 0.5, 0) 
	Frame.Size = UDIM2(1, 0, 0, 1) 

	Frame_2.Parent = ScreenGui 
	Frame_2.BackgroundColor3 = COL3RGB(167, 29, 196) 
...
world:Element("Dropdown", "hitsound", {options = {"none", "osu", "Tf2", "Slap", "1", "Minecraft", "jojo", "vibe", "supersmash", "epic", "retro", "quek"}}) 
...
movement:Element("Toggle", "no launch")
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "Astolfo is a waifu" or "Semi is a cool guy", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"standard", "LunarSense"}}) 
	...
	if PlayerIsAlive then 
				...
				for _,Player in pairs(Players:GetPlayers()) do 
					...
					if Player.Character and Player.Character:FindFirstChild("Humanoid") and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and Player ~= LocalPlayer then 
						if TBLFIND(values.rage.aimbot.resolver.Jumbobox, "pitch") then 
							Player.Character.UpperTorso.Waist.C0 = CFAngles(0, 3, 0) 
						end 
		...
		if UserInputService:IsKeyDown("Space") and values.misc.movement["bunny hop"].Toggle then 
			...
			if add == 0 and values.misc.movement.direction.Dropdown == "directional" and not UserInputService:IsKeyDown("W") then 
				...
				if values.misc.movement.type.Dropdown == "cframe" then 
					BodyVelocity:Destroy() 
					Root.CFrame = Root.CFrame + Vec3(rot.LookVector.X,0,rot.LookVector.Z) * values.misc.movement["speed"].Slider/99 
				end 
			end 
		end 
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
			...
			if values.rage.angles["body roll"].Dropdown == "180" then 
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "180" and RAD(180) or 0, 16, 0) 
